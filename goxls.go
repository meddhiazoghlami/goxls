// Package goxls provides a lightweight, high-performance library for reading
...
//
//	workbook, err := goxls.ReadFile("data.xlsx")
//	workbook, err := goxls.ReadFile("data.xlsx", goxls.WithParallel(true))
//	workbook, err := goxls.ReadFile("data.xlsx", goxls.WithMinColumns(3), goxls.WithMinRows(5))
...
// Example:
//
//	sheet, err := goxls.ReadSheet("data.xlsx", "Sales")
...
// Example:
//
//	json, err := goxls.ToJSONPretty(table)
...
// Example:
//
//	csv, err := goxls.ToCSVWithDelimiter(table, ';')
...
// Example:
//
//	sql, err := goxls.ToSQL(table, "users")
...
// Example:
//
//	sql, err := goxls.ToSQLWithCreate(table, "users")
...
// Example:
//
//	template := goxls.NewTemplate().
//	    RequireSheets("Sales", "Inventory").
//	    WithSchema("Sales", goxls.NewSchema().
//	        RequireColumns("ID", "Product", "Price").
//	        WithColumnTypes(map[string]goxls.CellType{
//	            "ID":    goxls.CellTypeNumber,
//	            "Price": goxls.CellTypeNumber,
//	        }).
//	        Build()).
//	    Build()
//
...
// Example:
//
//	template := goxls.NewTemplate("MyTemplate").
...
// Example:
//
//	result := goxls.QuickValidate(workbook, "ID", "Name", "Email")
//	if !result.Valid {
...
// Example:
//
//	missing := goxls.ValidateColumns(table, "ID", "Name", "Email")
//	if len(missing) > 0 {
...
//	code, err := goxls.GenerateStruct(table, "Person")
//	// Output:
//	// type Person struct {
...
// Example:
//
//	opts := goxls.DefaultSchemaOptions("Person")
//	opts.JSONTags = true
//	code, err := goxls.GenerateStructWithOptions(table, opts)
